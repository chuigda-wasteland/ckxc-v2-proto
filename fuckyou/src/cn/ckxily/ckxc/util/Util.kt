...
fun main(args: Array<String>) {
	fun useEither(either: Either<String, Int>) {
		val intResult: Int? = when (either) {
			// here `either.obj` is String, and you can convert it to a char list
			is Left -> {
				println(either.obj.toCharArray().toList())
				either.obj.toIntOrNull()
			}
			// here `either.obj` is Int, and you can plus 1 on it
			is Right -> {
				println(either.obj + 1)
				either.obj
			}
			// no else branch needed, while `when` is an expression
			// exhaustiveness check is awesome
		}
