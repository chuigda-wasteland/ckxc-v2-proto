	...
	run {
		...
		run {
			...
			run {
				...
				enum1.pushDecl(enumerator1)
				enum1.pushDecl(enumerator2)
				null
			}
			val var1 = VarDecl("light1k", ClassType(class2), class1)
			val var2 = VarDecl("activeGroup", EnumType(enum1), class1)

			class1.pushDecl(class2)
			class1.pushDecl(enum1)
			class1.pushDecl(var1)
			class1.pushDecl(var2)
			null
		}
		val var3 = VarDecl("ice1k", ClassType(class1), transUnit)

		transUnit.pushDecl(class1)
		transUnit.pushDecl(var3)
		null
	}
